...
```text
用户输入："帮我加上：生抽一瓶500毫升，陈醋一瓶420毫升"
  ├── 意图识别：主意图 期望 `inventory_add` · 实际 `inventory_add` ✓；多意图 期望去重=['inventory_add'] ⊆ 实际（顺序无关）`['inventory_add']` 召回=1.00 ✓
  ├── 槽位提取：restock_items 期望含 `[{'name': '生抽', 'amount': 500, 'unit': 'ml', 'merge_mode': 'add'}, {'name': '陈醋', 'amount': 420, 'unit': 'ml', 'merge_mode': 'add'}]` 实际 `['{"name": "生抽", "amount": 500, "unit": "毫升"}', '{"name": "陈醋", "amount": 420, "unit": "毫升"}']` ✗
...
- **效率 §5.4（观测，不计入 overall）**：`metrics` wall_ms=8518.747 tokens=None mcp≈0
