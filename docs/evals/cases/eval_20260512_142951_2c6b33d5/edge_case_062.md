...
```text
用户输入："更新一下我喜欢酸甜口讨厌过重花椒味"
  ├── 意图识别：主意图 期望 `profile_sync` · 实际 `profile_sync` ✓；多意图 期望去重=['profile_sync'] ⊆ 实际（顺序无关）`['profile_sync']` 召回=1.00 ✓
  ├── 槽位提取：profile_fragments 期望含 `['喜欢酸甜', '讨厌重花椒']` 实际 `['喜欢酸甜口', '讨厌过重花椒味']`, profile_explicit=? ✗
...
- **效率 §5.4（观测，不计入 overall）**：`metrics` wall_ms=9789.359 tokens=None mcp≈0
