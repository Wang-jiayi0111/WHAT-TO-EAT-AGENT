...
用户输入："想做糖醋鲤鱼，还差哪些食材把购物清单列给我"
  ...
  ├── 过滤排序：—（未实现）
  └── 生成回复：末轮可见文本片段「已按**最新库存**与菜谱用料重新计算缺口，待购清单如下：    · 鲤鱼：1500.0 g   · 清水：50.0 ml   · 番茄酱：40.0 g   · 白糖：20.0 g   · 白醋：10.0 ml   · 淀粉：210.0 g…」✓
```
...
| golden_recipe_ids | `['recipe_id_糖醋鲤鱼']` | `recipe_id_糖醋鲤鱼:命中@1` |

## §5.1～5.4 分项与用例总分

- **overall（§5.6 加权）**：`0.855375`

- **检索层 §5.1**：aggregate=`1.0000`
- **生成层 §5.2**：aggregate=`1.0000`
- **对话层 §5.3**：aggregate=`0.8215`
- **效率 §5.4（观测，不计入 overall）**：`metrics` wall_ms=62757.315 tokens=None mcp≈2
