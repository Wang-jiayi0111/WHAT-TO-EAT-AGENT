...
**DEV-NNN**：全局自增流水号，功能开发与缺陷修复共用序列，从 DEV-001 起**连续编号**，与开发计划 `T-xxx` **无一一对应**（例如任务 T-009 记录在 [DEV-011]）。新增条目取 **当前最大 DEV 编号 + 1**（截至仓库现状，最后一条为 [DEV-021]，下一条应为 **DEV-022**）。`docs/dev_log.md` 正文里各节应按 **DEV 数值递增**排版；若误用了大号（历史曾出现跳至 DEV-030），应统一改正文标题与全库交叉引用，而非保留空洞号段。
