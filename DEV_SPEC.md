...
本项目旨在开发一个**家庭膳食决策与库存管理 Agent**。它不是泛化的生活助理，而是运行在**本地菜谱库**、**个人饮食约束**与**家庭食材库存**三个可控状态上的任务型 Agent，通过 **MCP Server** 模式集成菜谱 RAG 知识库，并围绕「找菜谱 → 锁定权威菜谱 → 解析食材需求 → 比对库存 → 生成购物缺口 / 扣减库存」形成可复现、可解释、可评测的闭环。

| 维度 | 技术口径 |
|------|----------|
| **输入** | 用户自然语言请求，包括菜谱推荐、指定菜名检索、库存查询、购物清单索要、补货录入、做饭后扣减库存等。 |
...
| **输出** | 菜谱候选、澄清问题、权威菜谱解析结果、结合约束的推荐理由、库存查询结果、购物缺口清单、补货预览、确认后的库存变更反馈。 |
| **验证** | `eval/` 固定用例集进行端到端采集与分层打分，覆盖检索、生成、对话、多意图、库存与购物清单等行为。 |
...
- **基于线程的持久化：**
  ...
  - **“滑动窗口 + 语义压缩”机制：**
    - **滑动窗口机制**：LLM 热上下文仅保留最近 `memory.summary.window_size` 条原始对话消息（当前配置默认 4），以保证对即时指令（如“刚才说的那个菜...”）的精准指代消歧。
    - **语义摘要：**当 `thread_id` 下的原始消息数超过 `memory.summary.compress_trigger`（当前配置默认 8）时，`conversation_summary` 节点将窗口外历史记录压缩为 L2 摘要，并写入 `conversation_summary` / `memory_state.conversation_summary`。
    - **非损压缩原则 (Logistics-Aware Compression)**：压缩只处理自然语言消息；涉及工具调用、写库决策和后勤计算的结构化事实必须保留在对应状态切片中，不得被摘要替代。

  - **上下文管理三问：**

    | 问题 | 技术口径 |
    |------|----------|
    | **什么必须保留** | `task_stack`、槽位、澄清候选、锁定菜谱、`recipe_file_ref`、结构化食材需求 **R**、库存快照 **I**、`cached_shopping_gap`、`gap_basis`、`shopping_list_overlay`、补货/扣减预览与确认状态、错误码、硬约束与仍在 TTL 内的短期约束。 |
    | **什么可以摘要** | 已完成或非阻塞的闲聊、解释过程、推荐理由、历史偏好描述、普通问答背景等自然语言上下文。摘要只作为语义背景，不作为工具调用和写库的权威依据。 |
    | **什么时候压缩** | `len(messages) > memory.summary.compress_trigger` 时压缩窗口外消息；任务结束后、长回复后或上下文预算接近上限时可触发等价压缩，但必须保留业务切片。 |

    `conversation_summary` 节点的写入边界必须保持窄化：只裁剪 `messages` 并更新摘要，不清空 `control_state`、`recipe_state`、`inventory_state`、`error_state`。摘要失败时保留原热窗口继续主流程。
- **短期记忆的状态模式：**
  
  短期工作记忆在代码层表现为一个名为 `AgentState` 的结构化字典，所包含的字段如 3.6.8 中的 **AgentState 核心字段定义**所示。
- **对推理过程的增强：**
...
- **动态更新与冲突处理：**
  - **身份动态切换：**支持在同一线程内通过指令切换 `active_user_id`；切换后，系统立即应用新成员的画像约束（如过敏原过滤），同时保留该线程之前的对话上下文（如刚才讨论的菜谱），实现“换人不换话题”。
  - **语义记忆动态更新：**
    ...
    3. **生成新简报**：由 `conversation_summary` 节点生成更新后的“全量语义快照”。
