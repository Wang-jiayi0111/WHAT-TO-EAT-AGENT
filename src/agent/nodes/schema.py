...
class IntentResult(BaseModel):
    ...
    #  List 结构以支持多意图（与 §11.3、router.INTENT_TASK_MAPPING 对齐）
    ...
    ]] = Field(description="识别出的意图列表（§11.3 目录）")
    ...
    entities: Dict[str, Any] = Field(
        ...
        description="兼容迁移期的原始实体（路由层归一为 §11.2 slots）",
    )

    slots: Dict[str, Any] = Field(
        default_factory=dict,
        description="§11.2 全局槽位命名空间（可与 entities 并存；路由会与 entities 合并）",
    )

    missing_slots: List[str] = Field(
        default_factory=list,
        description="模型自检的本轮仍缺的槽位名（路由会与规则校验 §11.5 合并去重）",
    )
